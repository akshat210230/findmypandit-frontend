...
const REVIEWS = [
  ...
  { name: 'Sneha Patel', city: 'Indore', text: 'As someone new to the city, finding a trusted pandit felt impossible. This app changed everything for our family.', rating: 5 },
]
    ...
    <div style={{ background: '#FFFAF5', minHeight: '100vh' }}>
      ...
      <section className="relative overflow-hidden" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div className="absolute inset-0" style={{
          background: 'linear-gradient(180deg, #FFF5EC 0%, #FFFAF5 40%, #FEF0E4 100%)'
        }} />

        {/* Spiritual mandala pattern — very subtle */}
        <div className="absolute inset-0 flex items-center justify-center" style={{ opacity: 0.04, transform: `translateY(${scrollY * 0.08}px)` }}>
          <svg viewBox="0 0 500 500" style={{ width: 700, height: 700 }} fill="none" stroke="#C05818" strokeWidth="0.6">
            ...
            {[...Array(16)].map((_, i) => (
              <ellipse key={i} cx="250" cy="90" rx="22" ry="55" transform={`rotate(${i * 22.5} 250 250)`} />
            ))}
            {[...Array(8)].map((_, i) => (
              <line key={`l${i}`} x1="250" y1="10" x2="250" y2="490" transform={`rotate(${i * 22.5} 250 250)`} />
            ))}
          </svg>
        </div>
        ...
        <div className="absolute inset-0" style={{
          opacity: 0.025,
          backgroundImage: 'radial-gradient(#C05818 1px, transparent 1px)',
        ...
        <div className={`relative z-10 max-w-5xl mx-auto px-6 text-center pt-24 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          {/* Heading */}
          <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 4.5rem)', lineHeight: 1.08, marginBottom: 20, color: '#2C1810' }}>
          ...
          <p className="text-lg sm:text-xl max-w-lg mx-auto mb-10 leading-relaxed" style={{ color: '#7A6350' }}>
      ...
      <section className="py-24 px-6 relative">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          <div className="flex items-end justify-between mb-12">
            ...
            <Link href="/services" className="hidden sm:block text-sm font-semibold transition-all hover:-translate-y-0.5" style={{ color: '#D4651E' }}>
              View all 15+ →
            </Link>
          </div>
            ...
            {SERVICES.map((s, i) => (
              <Link href="/services" key={i}
                ...
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-4"
                  style={{ background: 'rgba(212,101,30,0.06)' }}>{s.icon}</div>
                ...
                <div className="mt-4 text-sm font-semibold opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: '#D4651E' }}>
                  Book now →
                </div>
              </Link>
            ))}
      ...
      <section className="py-24 px-6 relative overflow-hidden">
        ...
        {/* Subtle Om pattern */}
        <div className="absolute top-10 right-10 opacity-[0.03]" style={{ transform: `translateY(${scrollY * 0.05}px)` }}>
          <svg viewBox="0 0 200 200" style={{ width: 300, height: 300 }} fill="none" stroke="#C05818" strokeWidth="1">
            {[...Array(8)].map((_, i) => <ellipse key={i} cx="100" cy="40" rx="18" ry="45" transform={`rotate(${i * 45} 100 100)`} />)}
          </svg>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
            <div>
              ...
              <h2 className="text-3xl sm:text-4xl font-bold mb-6" style={{ color: '#2C1810' }}>
                Book at the most<br />
                <span style={{ color: '#D4651E' }}>auspicious time</span>
              </h2>
              <p className="text-base leading-relaxed mb-8" style={{ color: '#7A6350' }}>
                Our built-in Choghadiya calculator shows the most auspicious time slots
                for your ceremony based on Vedic astrology. No more guessing.
              </p>
              <div className="space-y-3">
                {['Amrit, Shubh & Labh periods highlighted in green', 'Rog, Kaal & Udveg marked to avoid', 'Updates automatically for every date you pick'].map((f, i) => (
            ...
            <div className="p-6 rounded-2xl" style={{ background: '#FFFFFF', border: '1px solid rgba(180,130,80,0.1)', boxShadow: '0 8px 40px rgba(120,80,30,0.06)' }}>
              ...
              {[
                ...
                <div key={i} className="flex items-center gap-3 p-3 rounded-xl mb-2 transition-all"
                  style={{
                    background: i === 0 ? 'rgba(212,101,30,0.04)' : s.type === 'bad' ? 'rgba(197,48,48,0.03)' : 'rgba(0,0,0,0.01)',
            ...
            {REVIEWS.map((r, i) => (
              <div key={i} className="p-6 rounded-2xl transition-all hover:-translate-y-1"
                ...
                <div className="flex gap-0.5 mb-4">
                  {[...Array(r.rating)].map((_, j) => <span key={j} style={{ color: '#D4A017', fontSize: 14 }}>★</span>)}
                </div>
      ...
      <section className="py-24 px-6 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center relative z-10">
          <h2 className="text-4xl sm:text-5xl font-extrabold mb-5" style={{ color: '#2C1810' }}>
            All your ceremonies,
            <br />in one app.
          </h2>
          <p className="text-lg mb-10" style={{ color: '#7A6350' }}>
      ...
      <footer className="py-12 px-6" style={{ background: '#2C1810' }}>
        <div className="max-w-5xl mx-auto">
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm"
                  style={{ background: 'linear-gradient(135deg, #D4651E, #B8860B)' }}>🙏</div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center pt-6" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <span className="text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>© 2026 Find My Pandit. All rights reserved.</span>
